(function () {
  ...
  function textSystemInstruction() {
    return [
      ...
      'Если пользователь просит добавить текст, описание, абзац, заголовок, подпись или другой видимый текст — выбери type "text"; по умолчанию tag "p", для заголовка h1-h6, для короткой inline-подписи span.',
  ...
  function addImageCodeDraftMessage(parts, rawText = '', options = {}) {
    ...
    body.textContent = String(parts?.message || '').trim()
      || `Код по картинке разложен по вкладкам (${total} симв.).`;
  ...
  function parseStyleDeclarations(value) {
    ...
    String(value || '').split(';').forEach((part) => {
      ...
      if (index < 0) return;
      const name = normalizeCssPropertyName(part.slice(0, index));
      const cssValue = part.slice(index + 1).trim();
  ...
  function normalizeConstructorStyleDeclaration(name, value) {
    ...
    Object.entries(source).forEach(([rawName, rawValue]) => {
      ...
      const nextValue = normalizeConstructorLengthValue(nextName, rawValue);
      if (shouldDropConstructorStyleValue(nextName, nextValue)) return;
      result[nextName] = nextValue;
    });
    return result;
  ...
  function normalizeConstructorSvgMarkup(value) {
    ...
    [normalized].concat(children).forEach((element) => {
  ...
  function parseConstructorCssRules(cssText, media = 'screen', result = [], options = {}) {
    ...
    while (index < source.length) {
      ...
      } else if (selector && !selector.startsWith('@') && !body.includes('{')) {
  ...
  function addConstructorSourceCssClassStyles(ctx, cssSource, options = {}) {
    ...
    extractConstructorStyleBlocks(cssSource).forEach((block) => {
      parseConstructorCssRules(block, 'screen', [], { replaceRules }).forEach((rule) => {
  ...
  function buildConstructorEmbedStub(ctx, parentId) {
    ...
    ctx.tags[noteId] = {
      ...
      data: { text: { type: 'HTML', value: 'Этот код будет работать только на опубликованном/��кспортированном проекте' } },
      dataSource: 'from_parent',
      dataAccess: 7
    };
    return stubId;
  ...
  function buildConstructorClipboardData(spec, options = {}) {
    ...
    const embedCode = constructorEmbedCodeFromSpec(source);
