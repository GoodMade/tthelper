...
function isIgnorableTabError(error) {
  ...
  return /Frame with ID \d+ was removed|Frame with ID \d+ is showing error page|showing error page|No frame with id|No tab with id|The tab was closed|Cannot access contents of url|Cannot access contents of the page|Cannot access a chrome-extension:\/\/ URL of different extension|Cannot access a? ?chrome:\/\/ URL|The extensions gallery cannot be scripted|Cannot be scripted|Missing host permission|Receiving end does not exist/i.test(message);
}
...
function isCatalogUrl(url) {
  try {
    ...
    return parsed.hostname === 'localhost' ||
           ...
           // Только страницы НАШЕГО расширения, а не любого chrome-extension:// (иначе пытаемся инжектить в чужие расширения).
           (parsed.protocol === 'chrome-extension:' && parsed.hostname === (chrome.runtime?.id || ''));
  } catch (e) {
    return false;
  }
}
